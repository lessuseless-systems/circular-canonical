...
test-py-unit:
    ...
    @cd dist/circular-py && pip install -e . --quiet 2>/dev/null && pytest tests/test_unit.py -v

# Python: Integration tests (with mock server)
test-py-integration:
    @echo "🧪 Python: Integration tests"
    @cd dist/circular-py && pip install -e . --quiet 2>/dev/null && pytest tests/test_integration.py -v 2>/dev/null || echo "⏭️  Integration tests not configured"
...
test-py-e2e:
    ...
    @if [ -z "$$CIRCULAR_TEST_ADDRESS" ]; then \
        ...
        cd dist/circular-py && pip install -e . --quiet 2>/dev/null && pytest tests/test_e2e.py -v -m e2e; \
    fi
